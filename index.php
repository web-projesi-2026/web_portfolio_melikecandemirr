...
<head>
    ...
    <title>Tech-Timeline Panel | phpMyAdmin Entegrasyonlu</title>
...
<body>
    ...
    <header class="header">
        <div class="header-content">
            <div class="logo">
                ...
                <div class="logo-text"><h1>TECH-TIMELINE</h1><p>Yönetim Paneli</p></div>
            </div>
            <nav class="navigation">
                ...
                <ul class="nav-menu">
                    ...
                    <?php if(isset($_SESSION['user_id'])): ?>
                        ...
                        <li><a href="?cikis=1" class="nav-link" style="color:#ff7675;">🚪 Çıkış</a></li>
                    <?php endif; ?>
                </ul>
    ...
    <main style="padding: 2rem 10%;">
        <section id="weather-section" class="weather-card" style="background: linear-gradient(135deg, #00592D, #008943); color: white; padding: 20px; border-radius: 15px; margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 8px 20px rgba(0,0,0,0.15);">
            <div><h3>🌤️ Kahramanmaraş Afşin Hava Durumu (Canlı API)</h3><p id="weather-desc">Yükleniyor...</p></div>
        ...
        <?php else: ?>
            <section class="form-box" style="margin-bottom: 3rem; border-left: 6px solid #00592D;">
                <h3>🛠️ Veritabanına Yeni Robot Ekle (Create)</h3>
                <form method="POST" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-top: 20px;">
                    ...
                    <div class="form-group"><span>🖼</span><input type="text" name="resim" placeholder="Resim URL" required></div>
                    <div class="form-group" style="grid-column: 1/-1;"><span>📝</span><textarea name="aciklama" placeholder="Teknik Analiz" required style="height: 80px;"></textarea></div>
            ...
            <?php foreach($robotlar as $robot): ?>
                <div class="card robot-item" data-title="<?= strtolower($robot['baslik']) ?>" data-desc="<?= strtolower($robot['aciklama']) ?>">
                    ...
                    <div style="padding:20px; border-top:1px solid #eee; display:flex; justify-content:space-between; align-items:center;">
                        <span style="font-weight:bold;">📅 <?= $robot['yil'] ?></span>
                        <div style="display:flex; gap:8px;">
                            <?php if(isset($_SESSION['user_id'])): ?>
                                ...
                                <button onclick="openEditForm(<?= $robot['id'] ?>)" style="background:#00592D; color:white; border:none; padding:5px 10px; border-radius:5px; font-weight:bold; cursor:pointer;">Düzenle</button>
                                <a href="?sil=<?= $robot['id'] ?>" onclick="return confirm('Silinsin mi?')" style="background:#ff7675; color:white; text-decoration:none; padding:5px 10px; border-radius:5px; font-weight:bold; font-size:0.85rem;">Sil 🗑️</a>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div id="edit-form-<?= $robot['id'] ?>" style="display:none; padding:20px; background:#f9f9f9; border-top:2px solid #d4af37;">
                        <form method="POST">
                            ...
                            <div style="display:flex; gap:10px;"><button type="submit" name="robot_guncelle" style="background:#2ecc71; color:white; border:none; padding:8px; cursor:pointer; flex-grow:1;">Kaydet</button><button type="button" onclick="closeEditForm(<?= $robot['id'] ?>)" style="background:#95a5a6; color:white; border:none; padding:8px;">İptal</button></div>
                        </form>
                    </div>
                </div>
            <?php endforeach; ?>
    ...
    <script>
        document.getElementById('live-search').addEventListener('keyup', function(e) {
            ...
            document.querySelectorAll('.robot-item').forEach(item => {
                if (item.getAttribute('data-title').includes(aramaMetni) || item.getAttribute('data-desc').includes(aramaMetni)) { item.style.display = 'flex'; } else { item.style.display = 'none'; }
            });
        });
